...
final class Health {
    ...
    func begin(read: @escaping Read, write: @escaping Write) async throws {
        ...
        [Metric(identifier: .stepCount, unit: .count()) {
            $0.steps = $1
        },
         .init(identifier: .distanceWalkingRunning, unit: .meter()) {
             $0.distance = $1
         },
         .init(identifier: .activeEnergyBurned, unit: .largeCalorie()) {
             $0.calories = $1
         }]
            .forEach { metric in
    ...
    private struct Metric: @unchecked Sendable {
        ...
        let unit: HKUnit
        let update: @MainActor (inout Walk, Int) -> Void
